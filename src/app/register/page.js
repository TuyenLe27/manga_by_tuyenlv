...
import { Lock, User, Loader2, BookOpen, KeyRound } from 'lucide-react';
      ...
      <div className="w-full max-w-md space-y-8 bg-slate-950 border border-slate-700/80 p-8 rounded-2xl shadow-xl">
        <div className="text-center">
          ...
          <p className="mt-1.5 text-xs text-slate-400">
            Đăng ký tài khoản để lưu truyện yêu thích và theo dõi lịch sử đọc.
          </p>
        </div>
        ...
        {success && (
          <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs text-emerald-400 text-center">
            🎉 {success}
          </div>
        )}
        ...
        <div className="text-center pt-2 text-xs text-slate-500">
          Đã có tài khoản?{' '}
          <Link href="/login" className="text-violet-400 hover:text-violet-300 font-medium underline transition-colors">
            Đăng nhập ngay
          </Link>
        </div>
      </div>
