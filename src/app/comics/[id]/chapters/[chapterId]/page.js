  ...
  useEffect(() => {
    const fetchReaderData = async () => {
      try {
        ...
        // Fetch chapter info and its images
        const chapterRes = await fetch(`/api/chapters/${chapterId}?t=${Date.now()}`);
        if (!chapterRes.ok) throw new Error('Không thể tải chương truyện này');
        ...
        // Fetch comic details to get other chapters list
        const comicRes = await fetch(`/api/comics/${comicId}?t=${Date.now()}`);
        if (!comicRes.ok) throw new Error('Không thể tải thông tin bộ truyện');
        const comicData = await comicRes.json();
        
        if (comicData.chapters) {
          // Sort chronologically (lowest number first)
          comicData.chapters.sort((a, b) => a.chapterNumber - b.chapterNumber);
        }
        setComic(comicData);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex min-h-[85vh] flex-col items-center justify-center gap-4 bg-slate-950 text-slate-100">
        ...
        <p className="text-sm text-slate-400">Đang tải trang truyện...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center select-none">
      ...
      <div className="sticky top-16 z-30 w-full border-b border-slate-700/80 bg-slate-950/90 backdrop-blur-md py-3 px-4">
        <div className="mx-auto max-w-4xl flex items-center justify-between gap-4">
          <Link
            href={`/comics/${comicId}`}
            className="flex items-center gap-1 text-slate-400 hover:text-slate-50 text-xs font-semibold"
          >
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        {[
                          ...
                          <button
                            ...
                            title={item.value === 'original' ? 'Hiển thị ảnh theo kích thước thật, ảnh nhỏ không bị phóng to làm bể hình' : 'Bắt buộc ảnh kéo rộng khớp với khung đọc'}
                          >
                            {item.label}
                          </button>
                        ))}
      ...
      <main className={`w-full ${readerWidth} bg-slate-950 flex flex-col py-2 transition-all duration-200`}>
        {(!chapter.images || chapter.images.length === 0) ? (
          ...
          <div className="relative w-full flex flex-col items-center bg-slate-950">
            {/* Click zones on desktop to navigate easily, overlays at left & right */}
            <div 
