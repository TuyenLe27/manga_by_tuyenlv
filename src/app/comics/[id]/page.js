...
export default function ComicDetailPage() {
  ...
  useEffect(() => {
    const fetchComicAndFavorite = async () => {
      try {
        const { id } = params;
        const res = await fetch(`/api/comics/${id}?t=${Date.now()}`);
        if (!res.ok) throw new Error('Không tìm thấy bộ truyện tranh này');
        ...
        const readHistory = localStorage.getItem(`read-history-${id}`);
        if (readHistory) {
          try {
  ...
  // Get chronological first and latest chapters
