...
export default function NewChapterPage() {
  ...
  // List of all items (newly added files) in sorted order
  ...
  useEffect(() => {
    const fetchComic = async () => {
      try {
        ...
        if (!res.ok) throw new Error('Không tìm thấy thông tin truyện');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'Lỗi khi đăng chapter mới');
      }

      setSuccess(`Đã đăng thành công Chapter ${chapterNumber}!`);
      setTimeout(() => {
        router.push('/admin');
        router.refresh();
      }, 1500);
  ...
  if (loading) {
    return (
      <div className="flex min-h-[40vh] flex-col items-center justify-center gap-4">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-slate-750 border-t-violet-500"></div>
  ...
  if (error && !comic) {
    return (
      <div className="max-w-md mx-auto text-center p-8 border border-red-500/20 bg-red-500/10 rounded-lg">
        ...
        <Link
          href="/admin"
          className="mt-4 inline-block text-xs bg-slate-800 text-slate-50 px-4 py-2 rounded-lg hover:bg-slate-700 transition-colors"
        >
          Quay lại Dashboard
        </Link>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 border-b border-slate-705 pb-4">
        ...
        <div>
          <span className="text-xs font-bold text-violet-500 uppercase tracking-widest">Đăng chương mới</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-slate-950 border border-slate-700/80 p-6 rounded-xl backdrop-blur-sm shadow-sm">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-semibold text-slate-350 block">Tải ảnh các trang truyện <span className="text-red-500">*</span></label>
            <p className="text-[11px] text-slate-500 mt-1">
              💡 Bạn có thể kéo thả để thay đổi vị trí, nhấn mũi tên Lên/Xuống hoặc xóa riêng lẻ từng trang ảnh bên dưới.
            </p>
          </div>
              ...
              {items.map((item, idx) => (
                ...
                >
                  ...
                  <div className="relative aspect-[3/4] w-full bg-slate-900 overflow-hidden">
                    ...
                    {/* Index Badge */}
                    <div className="absolute top-2 left-2 rounded text-[10px] font-bold px-1.5 py-0.5 shadow-md bg-fuchsia-600 text-white shadow-fuchsia-600/20">
                      Trang {idx + 1}
                    </div>
                    ...
                    <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-between p-2.5 transition-opacity duration-200">
                      ...
                      <button
                        ...
                        className="rounded-full bg-red-650 hover:bg-red-500 p-2 text-white shadow-lg active:scale-90 transition-transform cursor-pointer"
                        title="Xóa trang này"
                      >
                  ...
                  <div className="p-2 flex-1 flex flex-col justify-center bg-slate-950 text-left border-t border-slate-900">
                    <p className="font-semibold text-slate-350 text-[10px] truncate" title={item.name}>
                      {item.name}
                    </p>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>Đang upload {items.length} ảnh...</span>
              </>
            ) : (
              <span>Đăng Chapter</span>
            )}
          </button>
        </div>
      </form>
