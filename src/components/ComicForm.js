...
export default function ComicForm({ initialData = null, onSubmit, isSubmitting = false }) {
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      ...
      setCompressing(true);
      try {
        const compressedFile = await compressImage(file, 600, 800, 0.75); // Comic thumbnail can be smaller
        setThumbnailFile(compressedFile);
        setThumbnailPreview(URL.createObjectURL(compressedFile));
      } catch (err) {
        setValidationError('Lỗi khi nén ảnh: ' + err.message);
      } finally {
            ...
            <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4 text-center">
              {compressing ? (
                <>
                  ...
                  <p className="text-xs font-semibold text-slate-400">Đang tối ưu...</p>
                </>
              ) : (
                <>
                  <Upload className="h-8 w-8 text-slate-500 mb-2" />
                  <p className="text-xs font-semibold text-slate-400">Chọn ảnh bìa</p>
                  <p className="text-[10px] text-slate-450 mt-1">Tự động nén và tối ưu hóa dung lượng</p>
                </>
              )}
            </div>
            <input
